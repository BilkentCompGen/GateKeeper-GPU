...
double time_io_encoding, time_gkgpu; // GateKeeper-GPU time from host side
...
/* Reader Function */
int reader(FILE *in_file, long int batch_size, int *eof, int *file_index, int *remaining_batch_size, 
			int last_index, int device, RES_NODE *new_node, int start_index)
{
	char *line = NULL;
	size_t line_size = 0;
	...
	while(batch_counter < batch_size){
		...
		if (getline(&line, &line_size, in_file) == -1 || (*file_index) == last_index){ // eof or end of portion
			...
			str_status = host_encode_2bit(&line[line_tracer], READ_LENGTH, &h_read_buffer[device][(start_index + batch_counter) * WINDOW_COUNT]);
			if (str_status == UNDEFINED_STR){ // undefined read or ref
				filt[(start_index + batch_counter)] = PASS;
				eds[(start_index + batch_counter)] = UNDEFINED_STR_EDIT;
			}
			else {
				line_tracer += READ_LENGTH + 1;
				str_status = DEFINED_STR;

				str_status = host_encode_2bit(&line[line_tracer], READ_LENGTH, &h_ref_buffer[device][(start_index + batch_counter) * WINDOW_COUNT]);
				if (str_status == UNDEFINED_STR){ // undefined read or ref
					filt[(start_index + batch_counter)] = PASS;
					eds[(start_index + batch_counter)] = UNDEFINED_STR_EDIT;
				}
						...
						int numThreads)
{
	...
	int reader_status = SUCCESSFUL_MISSION, eof = 0, r_batch_size = 0; // remaining batch size
	...
	double time_io_encoding_start = 0, time_gkgpu_start = 0;
	time_io_encoding = 0, time_gkgpu = 0;
	...
	time_io_encoding += dtime() - time_io_encoding_start;
	...
	do {
		...
		for (thread = 0; thread < numThreads; thread++){
			...
			thr_reader_status = reader(thr_file, thr_portion_size, &thr_eof, &thr_file_index, &thr_r_batch_size, 
									finish_indices[thread]+1, currentDevice, new_node, thread * portion_size);
...
double multiGPU_driver(char *inputfile, int error_threshold, int deviceCount_cmd, char *filter_file_name, char *edit_file_name)
{
	...
	do {
		...
		// Reading Procedure
		reader_status = reader(dev_file, batch_size, &eof, &dev_file_index, &r_batch_size, finish_indices[device]+1, device, new_node, 0);
	...
	if(argc < 5){
		...
		else
		{
			...
			if (deviceCount_cmd > MAX_GPU){
				...
				exe_mode = SINGLE;
				printf("\nRunning on single GPU mode, host_encoded...\n");
				...
				exe_mode = MULTIPLE;
				printf("\nRunning on multi-GPU mode(%d), host_encoded...\n", deviceCount_cmd);
			...
			if (kernel_analysis_mode == ON){
				...
				printf("0-Longest kernel filtration time (msecs): %f\n", longest_kernel_time); 
			}
			printf("\n1-GateKeeper-GPU time (from host) (secs): %10.3lf\n", time_gkgpu);	
			printf("2-IO-reading + encoding + data transfer time (secs): %10.3lf\n", time_io_encoding); // data transfer = filling unified memory buffers
			printf("3-Total filtration time (secs): %10.3lf\n", t_exe); 	//measures filtration from host side, wraps the while loop of all batches: encoding, edlib included
